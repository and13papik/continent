...
const Dashboard: React.FC<DashboardProps> = ({ state, updateState }) => {
  ...
  }, [state.incomeData, state.operationsData, activePeriod, activePeriodId]);
  ...
  const stats = useMemo(() => {
    ...
    return totals;
  }, [state.incomeData, state.ownerManualIncomes, state.operationsData, state.admins, activePeriod, activePeriodId]);
  ...
  }, [state.incomeData, state.operationsData, state.operators, activePeriod, activePeriodId, state.paidStatuses]);
